...
- 已明确要设计工作坊、咨询材料或社群活动：进入对应的 `practices/` 目录。
## 存储与命名
```text
notes/{YYYY}/{YYYYMM}/
```
...
如果搜索结果中有多个名为 `notes` 的文件夹，只选择位于记录系统根目录下的 `notes/`。如果搜索结果为 0，则直接在飞书 Drive 根目录创建新的 `notes/`：
...
判断可以是：
- 仍在生活和思考中发生 → 留在原记录中；
- 已准备通过写作、社群、工作坊或咨询接受真实反馈 → 进入对应的 `practices/` 目录；
